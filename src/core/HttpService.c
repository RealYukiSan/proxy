...
struct HTTPInfo HTTPSClient(const char* website) {
    ...
    if (sockfd < 0) {
        printf("[HTTP CLIENT] Failed to create connection\n");
        exit(1);
    }
    
    server = gethostbyname(website);
    if (server == NULL) {
        printf("[HTTP CLIENT] NO HOST FOUND\n");
        exit(0);
    }
    ...
    if (connect(sockfd,(struct sockaddr *)&serv_addr,sizeof(serv_addr)) < 0) {
        printf("[HTTP CLIENT] Error: connecting\n");
        exit(0);
    }
...
void* HTTPSServer(void *unused) {
    ...
    if (!server_ctx) {
        printf("[HTTP SERVER] Failed to create server context");
        exit(1);
    }
    ...
    if (!SSL_CTX_check_private_key(server_ctx)) {
        printf("[HTTP SERVER] Private key not loaded\n");
        exit(2);
    }
    ...
    while(1) {
        ...
        if (client_sock < 0) {
            printf("[HTTP SERVER] Accept failed\n");
            exit(-3);
        }

        SSL* client = SSL_new(server_ctx);
        if (!client) {
            printf("[HTTP SERVER] Error Creating SSL Client");
            exit(-4);
        }
        ...
        if (SSL_accept(client)) {
            if (SSL_write(client, msg, strlen(msg)) < 0) 
                printf("[HTTP SERVER] Error in SSL Write\n");
        }   else 
                printf("[HTTP SERVER] SSL handshake\n");
        SSL_shutdown(client);
        usleep(500);
